...
google_chart_sparkline(Records, Background) ->
    Data = lists:foldl(fun(A, "") -> A; (A, Acc) -> Acc ++ "," ++ A end, "", [integer_to_list(X)|| X <- Records]),
    Min = lists:min(Records) - 2,
    Max = lists:max(Records) + 2,
    lists:concat(["http://chart.apis.google.com/chart?chs=400x200&cht=bvs&chd=t:", Data, "&chco=4d89f9&chf=&chf=bg,s,", Background ,"|c,s,", Background]).
...
handle_call({record, Name}, From, State) ->
    {Date, _} = calendar:universal_time(),
    handle_call({record, Name, Date}, From, State);

handle_call({record, Name, Date}, _From, State) ->
...
handle_call({graph, Name, Range}, From, State) when is_integer(Range) ->
    {_, Stats, _} = twoorl_stats:handle_call({stats, Name, Range}, From, State),
    ImgUrl = twoorl_stats:google_chart_sparkline(lists:reverse([Y || {X, Y} <- Stats])),
    {reply, ImgUrl, State};
